...
class ET_Core_Portability {
		...
		if ( $temp_file ) {
			...
			if ( 'post_type' === $this->instance->type ) {
				...
				foreach ( $data as $post ) {
					...
					if ( $gutenberg_format ) {
						// For Divi 5 content.
						$preset_ids = self::_get_preset_ids_from_d5_content( $post->post_content );

						// We'll look for used presets and get the presets data and set it to $used_global_presets.
						foreach ( $preset_ids as $preset_id ) {
							self::_prepare_used_presets_data_from_d5_content( $used_global_presets, $d5_presets, $preset_id );
						}
	 ...
	 * @return array
	 */
	protected static function _get_preset_ids_from_d5_content( $content ) {
		$pattern = '/"modulePreset":"(.*?)"/';
	 ...
	 * @return void
	 */
	protected static function _prepare_used_presets_data_from_d5_content( &$used_presets, $d5_presets, $preset_id ) {

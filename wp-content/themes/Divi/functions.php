...
function fitcoach_custom_button_shortcode($atts) {
    ...
    return  '<a href="' . esc_url($atts['url']) . '" style="display:inline-block;padding:10px 20px;border-radius:8px;text-decoration:none;color:' . esc_attr($atts['color']) . ';background:' . esc_attr($atts['bg']) . ';">' . esc_html($atts['text']) . '</a>';
}
...
function render_hero_section($atts, $content = null) {
    $atts = shortcode_atts(array(
        'badge_text' => 'Rated #1 Online Fitness Coach',
        'main_title' => 'Transform Your Body,',
        'highlight_title' => 'Transform Your Life',
        'paragraph' => '', // Custom paragraph attribute
        'primary_btn_text' => 'Start Your Journey',
        'primary_btn_url' => '#',
        'secondary_btn_text' => 'Watch Success Stories',
        'secondary_btn_url' => '#',
        'hero_image' => '',
        'stat1_number' => '5,000+',
    ...
    // Use paragraph attribute if provided, otherwise use content between shortcode tags, otherwise use default
    if (!empty($atts['paragraph'])) {
        $description = $atts['paragraph'];
    } elseif (!empty($content)) {
        $description = do_shortcode($content);
    } else {
        $description = 'Get personalized workout plans, nutrition guidance, and 24/7 support from certified fitness professionals. Start your transformation today.';
    }
...
function render_about_coach($atts, $content = null) {
    $atts = shortcode_atts(array(
        'main_title' => 'Meet Your',
        'highlight_title' => 'Dedicated Coach',
        'paragraph' => '',
        'coach_image' => '',
        ...
        // Quote
        'quote_text' => 'My mission is to empower individuals to unlock their full potential through sustainable fitness and nutrition habits that last a lifetime.',
        'quote_author' => 'Sarah Johnson',

        // Guarantee
        'guarantee_title' => 'Success Guarantee',
        'guarantee_text' => 'See results in 30 days or get your money back'
    ), $atts, 'about_coach');
    ...
    if (!empty($atts['paragraph'])) {
        ...
        $description = "I'm Sarah Johnson, a certified personal trainer with over 8 years of experience helping people achieve their fitness goals. My approach combines scientific training methods with personalized nutrition plans.";
    }
    ...
    ob_start();
    include get_template_directory() . '/template-parts/about-coach.php';
    return ob_get_clean();
}

...
class ET_Builder_Column extends ET_Builder_Structure_Element {
	...
	public function render( $atts, $content, $function_name ) {
		...
		// Check if flexbox experiment is enabled and modify column classes accordingly.
		$is_flexbox_enabled = et_get_experiment_flag( 'flexbox' );

		$flex_type = $this->props['flexType'] ?? '';

		if ( $is_flexbox_enabled ) {
			$this->add_classname( 'et_flex_column', 1 );
			$this->add_classname( 'et_flex_column_' . $flex_type, 1 );
		} else {
